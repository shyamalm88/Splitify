...
import { useExpense } from "../../services/expenseService";

const EditExpenseScreen = ({ navigation, route }) => {
  const { expenseId, groupId, expense: initialExpense, onSave } = route.params;
  ...
  // Fetch expense data if not provided in route params
  const { data: fetchedExpense, isLoading: isLoadingExpense } = useExpense(
    token,
    expenseId,
    { enabled: !initialExpense }
  );

  // Use either the provided expense or the fetched one
  const expenseData = initialExpense || fetchedExpense;
  const isLoading = isLoadingGroup || (isLoadingExpense && !initialExpense);
  ...
  useEffect(() => {
    if (expenseData) {
      ...
      // Set date - handle both string and Date objects
      if (expenseData.date) {
      ...
      // If we have custom splits, set mode to custom
      if (expenseData.splits && expenseData.splits.length > 0) {
        ...
        const equalShare = totalAmount / expenseData.splits.length;
        const isEqual = expenseData.splits.every(
          (split) => Math.abs(parseFloat(split.amount) - equalShare) < 0.01
        );
  ...
  // Initialize splits based on expense data and group members
  useEffect(() => {
    if (groupData?.members && expenseData?.splits) {
      // Map expense splits to member information
      const memberMap = {};
      groupData.members.forEach((member) => {
        memberMap[member.id] = member;
      });

      const expenseSplits = expenseData.splits.map((split) => ({
        user: split.user,
        name: memberMap[split.user]?.name || "Unknown",
        amount: parseFloat(split.amount),
      }));

      setSplits(expenseSplits);
    } else if (groupData?.members) {
      // If no splits data, create default splits
  ...
  }, [groupData?.members, expenseData?.splits]);
